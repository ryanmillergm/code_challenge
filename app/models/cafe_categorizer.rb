class CafeCategorizer
  ...
  def categorize_cafe(cafes_ordered_by_chairs)
    ...
    when 'ls2'
      categorize_by_chair_percentile(prefix, cafes_ordered_by_chairs)
    else
      @cafe.category = "other"
    end
  ...
  def categorize_by_chair_percentile(prefix, cafes_ordered_by_chairs)
    position = cafes_ordered_by_chairs.index(@cafe) + 1
    percentile  = (position.to_f / cafes_ordered_by_chairs.count) * 100
